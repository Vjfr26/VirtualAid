...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(
      `${LARAVEL_API_URL}/api/medico/${encodeURIComponent(medicoEmail)}/citas-canceladas-stats`,
      {
        method: 'GET',
        headers: {
    ...
    console.error('Error en el endpoint de citas canceladas:', error);
    return NextResponse.json(
      { 
        error: 'Error interno al obtener estadísticas de cancelaciones',
