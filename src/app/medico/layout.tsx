import { Metadata } from 'next'

export const metadata: Metadata = {
  title: 'Doctor Dashboard - VirtualAid | Medical Professional Portal',
  description: 'Doctor portal for VirtualAid. Manage patient appointments, medical records, and consultations.',
  keywords: 'doctor, medical professional, patient management, VirtualAid, healthcare portal',
  icons: {
    icon: [
      {
        url: 'data:image/svg+xml;base64,',
        sizes: '32x32',
        type: 'image/svg+xml',
      },
    ],
  },
}

export default function MedicoLayout({
  children,
}: {
  children: React.ReactNode
}) {
  return children
}
