import { Metadata } from 'next'

export const metadata: Metadata = {
  title: 'Terms of Service - VirtualAid | Service Agreement',
  description: 'VirtualAid terms of service. Understand the terms and conditions for using our medical platform.',
  keywords: 'terms of service, user agreement, medical services, VirtualAid, healthcare terms',
  icons: {
    icon: [
      {
        url: 'data:image/svg+xml;base64,',
        sizes: '32x32',
        type: 'image/svg+xml',
      },
    ],
  },
}

export default function TermsLayout({
  children,
}: {
  children: React.ReactNode
}) {
  return children
}
