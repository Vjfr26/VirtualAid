import { Metadata } from 'next'

export const metadata: Metadata = {
  title: 'Patient Dashboard - VirtualAid | Your Health Portal',
  description: 'Patient portal for VirtualAid. Book appointments, view medical history, and manage your healthcare.',
  keywords: 'patient portal, appointments, medical history, VirtualAid, healthcare management',
  icons: {
    icon: [
      {
        url: 'data:image/svg+xml;base64,',
        sizes: '32x32',
        type: 'image/svg+xml',
      },
    ],
  },
}

export default function UsuarioLayout({
  children,
}: {
  children: React.ReactNode
}) {
  return children
}
