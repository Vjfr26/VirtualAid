  ...
  return (
    <div className="bg-gradient-to-br from-white to-orange-50 rounded-xl shadow-lg p-6 border border-orange-100">
      <div className="flex items-center justify-between mb-6">
        ...
        <button 
          className="bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-lg font-medium flex cursor-pointer items-center space-x-2 transition-colors"
          onClick={onViewAll}
        >
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {especialistasInicio.slice(0, 6).map((medico, idx) => (
              <div 
                key={idx} 
                className={`group bg-white border border-gray-200 rounded-xl p-4 hover:shadow-lg hover:border-orange-300 transition-all duration-300 cursor-pointer transform hover:-translate-y-1 ${styles.specialistCard}`}
                ...
                <div className="flex items-center space-x-4">
                  <div className="relative">
                    {medico.avatar ? (
                      ...
                      <div className="w-14 h-14 bg-gradient-to-br from-orange-100 to-orange-200 rounded-full flex items-center justify-center border-2 border-orange-100 group-hover:border-orange-300 transition-colors">
                        <span className="text-orange-600 font-bold text-xl">
                          {medico.nombre?.charAt(0).toUpperCase() || 'D'}
                        </span>
                      </div>
                    )}
                    <div className={`absolute -bottom-1 -right-1 w-4 h-4 bg-green-500 rounded-full border-2 border-white ${styles.statusIndicator}`}></div>
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-gray-900 group-hover:text-orange-700 transition-colors">
                      <span className="truncate max-w-[160px] sm:max-w-[220px] inline-block" title={`${t('doctor_short')} ${medico.nombre} ${medico.apellido || ''}`}>
                        {t('doctor_short')} {medico.nombre} {medico.apellido || ''}
                      </span>
                    </h3>
                    <p className="text-orange-600 text-sm font-medium mb-1">{medico.especialidad}</p>
                  ...
                  <div className="flex flex-col items-center space-y-1">
                    <button
                      className="bg-orange-600 hover:bg-orange-700 cursor-pointer text-white px-3 py-1 rounded-lg text-xs font-medium transition-colors"
                      onClick={(e) => { 
                        e.stopPropagation(); 
