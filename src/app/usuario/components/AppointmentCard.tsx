      ...
      <div className={usuarioStyles.citaCardActions}>
        {!esPasada && (() => {
          ...
          return (
            <button
              type="button"
              className={`${usuarioStyles.citaCardAction} ${on ? usuarioStyles.citaCardActionSuccess : usuarioStyles.citaCardActionPrimary}`}
              onClick={() => toggleRecordatorio(cita)}
              aria-pressed={on}
              title={on ? (t('disable_reminder') as string) : (t('enable_reminder') as string)}
            >
              ...
              {on ? t('disable_reminder') : t('enable_reminder')}
            </button>
          );
        })()}
        {esPasada && (
